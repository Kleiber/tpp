...
Initializes a new solution with the name passed as a parameter. Basically a new directory is created with the cpp template `<solution-name>.cpp`, the input file `1.in` and the expected output file `1.exp`. If `TPP_FILL=1`, samples are auto-fetched from Codeforces or AtCoder.

```bash
tpp init <solution-name>
tpp init 1950A          # Codeforces (contest + problem letter)
tpp init abc466_a       # AtCoder (contest_task)
```
...
Compiles and executes the generated cpp template within the solution. Without a case number, runs interactively. With a case number, runs with that input file showing debug output.
...
Compiles, executes and test the generated cpp template within the solution. For the execution of this command it is necessary to have the `1.in` file with the input cases and the `1.exp` file with the expected outputs. From the cpp template an `1.out` file will be generated to be compared (`build < 1.in > 1.out`). Basically it is the translation of executing command `diff 1.exp 1.out`
