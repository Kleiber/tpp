...
# define template for Linux and Windows
function write_linux_template() {
  cat > "$1/$2" <<-EOF
/**
...
# define template for Mac
function write_mac_template {
  cat > "$1/$2" <<-EOF
/**
*  Generated by tpp tool
*  File: $2
...
function get_filename() {
  ...
  local filename=''
  if isMac; then
    filename=$(perl -nle'print $& while m{name\s*=\s*\K[\w\s-._]+}g' $config)
  else
    filename=$(cat $config | grep -oP "name\s*=\s*\K[\w\s-._]+")
  fi
...
EOF
  ...
  if isMac; then
    write_mac_template $1 $filename $reference
  else
    write_linux_template $1 $filename $reference
  fi
